...
function AppContent() {
  ...
  const [benchmarkCopyStatus, setBenchmarkCopyStatus] = useState<string | null>(
    null,
  );
  ...
  const zigCompatibilitySuffix =
    runtimeDiagnostics?.engine === 'zig' && !runtimeDiagnostics.zigApiCompatible
      ? ' (mismatch)'
      : '';
  ...
  const buildXxh3Seed = (): HashOptions['seed'] | undefined => {
    const rawValue = seedMode === 'label' ? seedLabel.trim() : seedInput.trim();
  ...
  const xxh3SeedLabel = seedMode === 'label' ? 'XXH3 seed label' : 'XXH3 seed';
  ...
  const handleHashString = async () => {
    ...
    try {
      ...
      Alert.alert('Hash string failed', error?.message ?? 'Unknown error');
    } finally {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: palette.bg }]}>
      ...
      >
        ...
        {showZigRuntimeDetails ? (
          <>
            <Text
              ...
              style={[styles.runtimeBadgeLine, { color: palette.muted }]}
            >
              zig abi: {zigApiLabel}
            </Text>
            <Text
        ...
        {runtimeInfoError ? (
          <Text style={[styles.runtimeBadgeLine, { color: '#ef4444' }]}>
        ...
        >
          ...
          >
                ...
                <TextInput
                  ...
                  style={[
                    styles.benchmarkInput,
                    {
                      color: palette.text,
                      borderColor: palette.border,
                      backgroundColor: isDarkMode ? '#11151d' : '#f5f7fb',
                    },
                  ]}
            ...
            {benchmarkStatus ? (
              <Text
                ...
                style={[styles.placeholder, { color: palette.muted }]}
              >
                {benchmarkStatus}
              </Text>
            ) : null}
            {benchmarkResults.length > 0 ? (
              <View testID="benchmark-results" style={styles.resultBox}>
                {benchmarkStatus === 'Benchmark complete' ? (
                  <View style={styles.resultBoxActions}>
                    {benchmarkCopyStatus ? (
                      <Text
                        numberOfLines={1}
                        style={[styles.copyStatus, { color: palette.muted }]}
                      >
                        {benchmarkCopyStatus}
                      </Text>
                    ) : null}
                ...
                <Text
                  selectable
                  style={[styles.resultText, { color: palette.text }]}
                >
              ...
              {algorithms.map(algo => {
                ...
                return (
                  ...
                  >
                    <Text
                      style={{
                        color: active ? '#0b1120' : palette.text,
          ...
          >
            ...
            {isXxh3Algorithm ? (
              <>
                ...
                {xxh3SeedValue.length > 0 ? (
                  <Text
                    ...
                    style={[styles.optionValue, { color: palette.muted }]}
                    selectable
                  >
                    seed: {xxh3SeedValue}
                  </Text>
              ...
              <>
                ...
                />
                <View style={styles.encodingRow}>
                  {(['utf8', 'hex', 'base64'] as TKeyEncoding[]).map(enc => {
            ...
            >
              <View
                style={[
                  styles.checkboxBox,
                  {
                    borderColor: mmapEnabled ? palette.accent : palette.border,
              ...
              >
                {mmapEnabled ? <View style={styles.checkboxCheck} /> : null}
              </View>
          ...
          >
            ...
            {pickedFile ? (
              <View style={styles.fileInfo}>
                ...
                >
                  {formatBytes(pickedFile.size)} • {pickedFile.displayUri}
                </Text>
              </View>
            ) : (
          ...
          >
            ...
            {hash ? (
              ...
              <Text style={[styles.placeholder, { color: palette.muted }]}>
                {fileStatus ?? 'Pick a file and tap “Hash file”'}
              </Text>
            )}
          </View>
          ...
          >
            ...
            />
            <View style={styles.encodingRow}>
              {(['utf8', 'base64'] as THashEncoding[]).map(enc => {
            ...
            <View style={styles.actionRow}>
              <Pressable
                style={[
                  ...
                  {
                    backgroundColor: palette.accent,
                    opacity: textInput && !textLoading ? 1 : 0.6,
                  },
                ]}
              ...
              >
                {textLoading ? (
                  ...
                  <Text style={styles.buttonText}>Hash string</Text>
                )}
              </Pressable>
              {textLoading ? (
                <Pressable
