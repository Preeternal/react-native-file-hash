...
function normalizeWindowsFilePath(input: string) {
    return input.trim();
}
...
function App() {
    ...
    const [textInput, setTextInput] = useState('hello Windows');
    ...
    const hashText = async () => {
        ...
        try {
            ...
            setTextStatus(
                isAbortError(error) ? 'Cancelled' : errorMessage(error)
            );
        } finally {
    ...
    const hashSelectedFile = async () => {
        const normalizedPath = normalizeWindowsFilePath(filePath);
        if (!normalizedPath) {
        ...
        try {
            const start = Date.now();
            const digest = await fileHash(normalizedPath, {
                algorithm: selectedAlgo,
                hashOptions: buildHashOptions(),
                signal: controller.signal,
            });
    ...
    const runBenchmark = async () => {
        if (!BenchmarkFile?.createFile) {
            Alert.alert(
                ...
                'Rebuild the Windows example app first.'
            );
            return;
        }
        ...
        try {
            const path = await BenchmarkFile.createFile(sizeBytes);
        ...
        >
            ...
            >
                ...
                <TextInput
                    ...
                    placeholder="C:\\Users\\me\\file.bin or file:///C:/Users/me/file.bin"
                    placeholderTextColor={palette.muted}
                    autoCapitalize="none"
                    autoCorrect={false}
                    style={[
